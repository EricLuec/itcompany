...
public class TestManager {
    ...
    @Test
    public void testManagerAlterPosition() {
        DefaultPerson person = new DefaultPerson(20, "eric", SWE, sweManager, architecture, sweBuilding);
        assertEquals(Designer.toString(), sweManager.alterPosition(person, Designer));
    }

    @Test
    public void testManagerItemManagement() {
        DefaultPerson person = new DefaultPerson(20, "eric", SWE, sweManager, architecture, sweBuilding);
        assertEquals(true, sweManager.grantItem(keyboard, person));
        assertEquals(false, sweManager.grantItem(mouseTwo, person));
        assertEquals(true, person.listItemsInPosession(person));
